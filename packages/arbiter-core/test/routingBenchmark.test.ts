...
import { createArbiterRouter, runRoutingBenchmark, ROUTING_BENCHMARK } from '../src/index';

describe('ArbiterRouter — 50-case expert benchmark (B→C gate: ≥85% agreement)', () => {
  ...
  it('achieves ≥85% agreement with the expert routing labels', async () => {
    const result = await runRoutingBenchmark(createArbiterRouter());
