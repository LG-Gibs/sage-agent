...
const sig = (
  network: ArbiterSignals['network'],
  power: ArbiterSignals['power'],
  complexity: ArbiterSignals['complexity'],
  privacy: ArbiterSignals['privacy'],
  preference: ArbiterSignals['preference'],
): ArbiterSignals => ({ network, power, complexity, privacy, preference });

const route = (s: ArbiterSignals) => router.route({ signals: s, capability });

describe('ArbiterRouter — 10 distinct signal-combination scenarios', () => {
  it('1. offline → local', () => {
