...
import { DEFAULT_CLOUD_CATALOG, type CloudModelCatalog, type IArbiterRouter } from '../router';
 ...
 * tool orchestration. Each cycle re-routes via the ArbiterRouter, runs the
...
export class ReActLoop {
  ...
  private async *runCycle(
    ...
    const decision = this.deps.arbiter.route({
  ...
  private buildDescent(decision: RoutingDecision): Attempt[] {
    ...
    if (decision.target === 'cloud') {
      // Honor the model the ArbiterRouter chose as the entry point, then descend.
