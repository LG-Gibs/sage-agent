...
function ensureInDomain<K extends keyof SageSignals>(
  key: K,
  value: SageSignals[K],
): SageSignals[K] {
 ...
 * domain. Guarantees a valid `SageSignals` vector — the Phase 0 success
 * criterion "all five SageRouter signals return valid values at cold start."
 ...
 * offline (which the SageRouter treats as a hard local override).
 */
const SAFE_FALLBACK: SageSignals = {
  network: 'offline',
  power: 'low',
  complexity: 'simple',
  privacy: 'standard',
...
export async function readSignalsSafe(
  ...
  const failed: (keyof SageSignals)[] = [];
